...
### 残業申請のフロー
1. **`/overtime` で申請者を選択** → 履歴と「+ 新規申請」ボタン
2. **ステップ1**: 申請種別（事前／事後）→ 日時 → 現場名・作業内容を入力
   - 退勤打刻があれば終了時刻が自動セット
3. **ステップ2**: 確認画面で内容を最終チェック
4. **送信** → 詳細画面に遷移、申請完了
5. **管理者が `/admin/overtime` で承認**（または差戻し／却下、コメント付き）
6. **`/admin/overtime/report` で月次集計確認 → CSVダウンロード**
...
| バックエンド | Server Actions + Route Handlers（Server Components主体） |
| データベース | SQLite（デモ） / Postgres等に切替可能 |
| ORM | Prisma 6 |
| スタイル | plain CSS（CSS変数でデザイントークン化） |
| 型 | TypeScript |
| 開発サーバー | Turbopack |
| 管理者認証 | 環境変数で4桁PIN（HMAC-SHA256署名Cookie、未設定時はDEMO MODE） |
