...
import { AttendanceQueueRow } from "./queue-row";
...
export default async function AdminAttendancePage({
  ...
  const session = await requireManager("/admin/attendance");
    ...
    <>
      ...
      <main className="container-wide">
        <header className="header">
          <div>
            <h1 className="title">勤怠申請 承認キュー</h1>
            <span className="subtitle">
              残業以外の申請（欠勤・遅刻・早退・私用外出・有給・特別・振替）の承認
            </span>
          </div>
          <div className="ot-admin-actions">
            <Link href="/admin/paid-leave" className="link">
              有給付与
            </Link>
        ...
        <section className="section">
          ...
          <div className="ot-admin-toolbar">
            <div className="ot-filter-tabs" role="tablist" aria-label="ステータス">
              <Link
                href={`/admin/attendance?${filterCategory !== "all" ? `category=${filterCategory}` : ""}`}
              ...
              <Link
                href={`/admin/attendance?status=all${filterCategory !== "all" ? `&category=${filterCategory}` : ""}`}
          ...
          <div className="ot-admin-toolbar" style={{ marginTop: 8 }}>
            <div className="ot-filter-tabs" role="tablist" aria-label="カテゴリ">
              <Link
                href={`/admin/attendance${filterStatus === "all" ? "?status=all" : ""}`}
              ...
              {ATTENDANCE_CATEGORIES.map((c) => (
                <Link
                  key={c}
                  href={`/admin/attendance?category=${c}${filterStatus === "all" ? "&status=all" : ""}`}
