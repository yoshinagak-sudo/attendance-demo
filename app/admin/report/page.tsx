    ...
    <>
      ...
      <main className="container">
        <header className="header">
          <div>
            <h1 className="title">日報管理</h1>
            <span className="subtitle">{formatJSTYmd(today)}・部下の日報を確認</span>
          </div>
          <div className="ot-admin-actions">
            <Link href="/admin/report/month" className="link">月次サマリ</Link>
        ...
        <section className="section">
          <div className="section-head">
            ...
            <span className="section-sub tabular">{todaySubmitted.length} / {users.length} 名</span>
          </div>
          {todaySubmitted.length === 0 ? (
            <div className="ot-empty"><div className="ot-empty-title">本日提出された日報はまだありません</div></div>
          ) : (
            <div className="dr-history-list">
              {todaySubmitted.map((r) => (
                <Link key={r.id} href={`/admin/report/${r.id}`} className="dr-history-row">
