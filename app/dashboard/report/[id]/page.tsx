    ...
    <>
      <header className="dash-page-head">
        <div className="dash-page-head-main">
          <span className="dash-page-eyebrow">日報詳細</span>
          <h1 className="dash-page-title">日報詳細</h1>
          <span className="dash-page-sub">
            {formatJSTYmd(report.reportDate)}・{report.user.name}
          </span>
        </div>
        <div className="dash-page-side">
          <Link href="/dashboard/report" className="link">← 戻る</Link>
        </div>
      </header>
      ...
      <div className="ot-detail-card card">
        <div className="ot-detail-row">
          ...
          <span className={`badge dr-badge-${status}`}>
            {STATUS_LABEL[status]}
          </span>
        </div>
        <div className="ot-detail-row">
        ...
        {report.acknowledgedAt && (
          <div className="ot-detail-row">
            ...
            <span>
              ...
              <span className="num">
                {formatJSTYmdHm(report.acknowledgedAt)}
              </span>
            </span>
          </div>
        )}
          ...
          {report.items.map((it, idx) => (
            <li key={it.id} className="dr-item-card">
              <div className="dr-item-head">
                ...
                <span className="num dr-item-duration">
      ...
      {report.progressNote && (
        <section className="section">
          ...
          <div
            className="card"
            style={{ whiteSpace: "pre-wrap", padding: 16, lineHeight: 1.7 }}
          >
            {report.progressNote}
          </div>
        </section>
        ...
        <section className="section">
          ...
          <form action={acknowledgeReport} className="ot-form">
            ...
            <div className="ot-field">
              <label className="ot-field-label" htmlFor="ackComment">
                コメント（任意）
              </label>
              <textarea
                id="ackComment"
