...
import {
  ...
  deactivateVehicle,
  reactivateVehicle,
} from "@/app/admin/settings/vehicle/actions";

export const dynamic = "force-dynamic";

type SearchParams = Promise<{ saved?: string; error?: string }>;

export default async function DashboardSettingsVehiclePage({
  searchParams,
}: {
    ...
    <>
      <header className="dash-page-head">
        <div className="dash-page-head-main">
          <span className="dash-page-eyebrow">車両マスタ</span>
          <h1 className="dash-page-title">車両マスタ</h1>
          <span className="dash-page-sub">
            社用車の追加・無効化・点検期限管理
          </span>
        </div>
        <div className="dash-page-side">
          <Link href="/dashboard/vehicle" className="link">
            稼働状況へ →
          </Link>
        </div>
      </header>
      ...
      <section className="ot-form-card">
        ...
        <form action={upsertVehicle} className="ot-form">
          ...
          <div className="ot-field">
            <label className="ot-field-label" htmlFor="inspectionDueDate">
              次回点検期限（任意）
            </label>
            <input
              id="inspectionDueDate"
          ...
          <div className="ot-field">
            <label className="ot-field-label" htmlFor="vehicleInspectionDueDate">
              次回車検期限（任意）
            </label>
            <input
              id="vehicleInspectionDueDate"
              name="vehicleInspectionDueDate"
              type="date"
              className="ot-input"
            />
      ...
      <section className="ot-form-card">
        ...
        {vehicles.length === 0 ? (
          <div className="ot-empty">
            <div className="ot-empty-title">車両が登録されていません</div>
          </div>
        ) : (
          <>
              ...
              <table className="table ot-site-table">
                ...
                <tbody>
                  {vehicles.map((v) => (
                    <tr
                      key={v.id}
                      className={v.isActive ? undefined : "is-inactive"}
                    >
                      <td>
                      ...
                      <td className="num">
                        {v.inspectionDueDate
                          ? formatJSTYmd(v.inspectionDueDate)
                          : "—"}
                      </td>
                      <td className="num">
                        {v.vehicleInspectionDueDate
                          ? formatJSTYmd(v.vehicleInspectionDueDate)
                          : "—"}
                      </td>
                      <td>
                        {v.isActive ? (
            ...
            <ul className="ot-site-card-list">
              {vehicles.map((v) => (
                <li
                  key={v.id}
                  className={`ot-site-card${v.isActive ? "" : " is-inactive"}`}
                >
                  <div className="ot-site-card-main">
                    ...
                    <div className="ot-site-card-meta">
                      ...
                      <span>{v.depot}</span>
                      {v.inspectionDueDate && (
                        <span>点検 {formatJSTYmd(v.inspectionDueDate)}</span>
                      )}
                      {v.vehicleInspectionDueDate && (
                        <span>
                          車検 {formatJSTYmd(v.vehicleInspectionDueDate)}
                        </span>
                      )}
                      {v.isActive ? (
                        <span className="ot-site-badge-active">有効</span>
